...
import { useStore, dateKey, getChapterName, revisionDate, revisionLabels, RevisionKey } from "@/lib/store";
...
export default function Dashboard() {
  ...
  const weakIds = new Set(state.feedback.filter((item) => item.confidence <= 2).map((item) => item.chapterId));
  const weakChapters = state.chapters.filter((chapter) => chapter.weakness.trim() || weakIds.has(chapter.id)).length;
  const confidentIds = new Set(state.feedback.filter((item) => item.confidence >= 4).map((item) => item.chapterId));
  const confident = state.chapters.filter((chapter) => confidentIds.has(chapter.id)).length;
  ...
  const cards = [
    ...
    { label: "Weak chapters", value: weakChapters, sub: "from notes and feedback", icon: AlertTriangle },
    { label: "Confident", value: confident, sub: "rated strong this week", icon: Flame },
  ];
      ...
      <section className="rounded-[2rem] bg-slate-950 p-8 text-white shadow-2xl shadow-emerald-100">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-slate-300">Add your own subjects and chapters, mark what you studied, and Tracked will build your revision plan for D1, D3, D7, D14, and D30.</p>
          </div>
          <div className="flex flex-wrap gap-3">
            <Link href="/syllabus"><Button className="rounded-full">Add syllabus</Button></Link>
            <Link href="/tasks"><Button variant="secondary" className="rounded-full">Plan today</Button></Link>
          </div>
        </div>
      </section>
